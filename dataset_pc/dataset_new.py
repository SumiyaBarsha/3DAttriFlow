...
class ShapeNet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        points = points.clone()
        if index >= len(self.data_metadata):
            raise IndexError(f"Index {index} out of range for dataset size {len(self.data_metadata)}")
        ...
        if self.opt.SVR:
            ...
            for v in range(num_views):
                img_path = os.path.join(return_dict['image_path'], ShapeNet.int2str(v) + ".png")
                im = Image.open(img_path).convert('RGB')
                if self.train:
                    im = self.dataAugmentation(im)
                else:
                    im = self.validating(im)
                im = self.transforms(im)
                image_list.append(im)

            multi_view_tensor = torch.stack(image_list, dim=0)  # (24, 3, H, W)
            return_dict['image'] = multi_view_tensor
        return return_dict
...
class ShapeNet_val(data.Dataset):
    ...
    def __getitem__(self, index):
        return_dict = deepcopy(self.data_metadata[index])
        # Point processing
        points = self.data_points[index].clone()
        if self.opt.sample:
            choice = np.random.choice(points.size(0), self.num_sample, replace=True)
            points = points[choice, :]
        return_dict['points'] = points[:, :3].contiguous()
        ...
        if self.opt.SVR:
            # Instead of loading a single image (like N or idx_image_val),
            # we load multiple images from 00.png to 23.png (24 views).
            image_list = []
            num_views = 24  # or however many you want
            for v in range(num_views):
                img_path = os.path.join(return_dict['image_path'], ShapeNet.int2str(v) + ".png")
                im = Image.open(img_path).convert('RGB')
                if self.train:
                ...
                im = self.transforms(im)           # scale to (3, H, W)
                image_list.append(im)

            # Stack along a new dimension => (V, 3, H, W)
            multi_view_tensor = torch.stack(image_list, dim=0)
            return_dict['image'] = multi_view_tensor
        return return_dict
...
if __name__ == '__main__':
    ...
    opt = {"normalization": "UnitBall", "class_choice": ["phone"], "SVR": True, "sample": True, "npoints": 2500,
           "shapenet13": True}
