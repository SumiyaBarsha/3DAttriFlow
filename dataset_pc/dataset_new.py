...
IMAGE_DIR = "/kaggle/input/shapenet-sub/data/ShapeNetV1Renderings"  # Path where multi-view images are stored
...
class MVP_CP(data.Dataset):
    def __init__(self, prefix="train", image_dir=IMAGE_DIR, transform=None):
        if prefix == "train":
            self.file_path = '/kaggle/working/3DAttriFlow/MVP_Train_CP.h5'
        elif prefix == "test":
            self.file_path = '/kaggle/working/3DAttriFlow/MVP_Test_CP.h5'
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.prefix != "test":
            ...
            for view_id in range(NUM_VIEWS):
                image_path = f"{self.image_dir}/{index}_view{view_id}.jpg"
                images.append(self.load_image(image_path))

            images = torch.stack(images)  # Stack multi-view images (Shape: [NUM_VIEWS, 3, 224, 224])
